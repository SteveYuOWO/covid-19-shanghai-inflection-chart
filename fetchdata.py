...
def get_dt_data(url, infection_datas):
    ...
    for link in soup.find_all('dt'):
        ...
        if ("上海新增" in link_str and "例本土确诊 " in link_str and "例无症状" in link_str and "时" not in link_str):
            infection_datas.append(link_str)
...
def get_all_infection_datas():
    infection_datas = []
    covidobjs = []
    for i in range(1, 99):
        print("fetch {}/{}...".format(i, 99))
        url = 'http://m.sh.bendibao.com/news/list_17_753_{}.htm'.format(i)
        get_dt_data(url, infection_datas)
    for data in infection_datas:
...
covidObjs = get_all_infection_datas()
